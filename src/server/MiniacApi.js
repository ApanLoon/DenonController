...
export class MiniacApi extends EventEmitter
{
    ...
    constructor(server)
    {
        ...
        this.signalR.hub("hub",
        {
            ampRequestPowerState     : ()      => this.emit(MiniacEvent.Amp_RequestPowerState),
            ampSetPowerState         : (isOn)  => this.emit(MiniacEvent.Amp_SetPowerState, isOn),
            ampRequestSelectedInput  : ()      => this.emit(MiniacEvent.Amp_RequestSelectedInput),
            ampSetSelectedInput      : (input) => this.emit(MiniacEvent.Amp_SetSelectedInput, input),
            playerRequestStatus      : ()      => this.emit(MiniacEvent.Player_RequestStatus),
            playerRequestCurrentSong : ()      => this.emit(MiniacEvent.Player_RequestCurrentSong),
            playerPlay               : ()      => this.emit(MiniacEvent.Player_Play),
            playerPause              : ()      => this.emit(MiniacEvent.Player_Pause),
            playerStop               : ()      => this.emit(MiniacEvent.Player_Stop),
            playerNext               : ()      => this.emit(MiniacEvent.Player_Next),
            playerPrev               : ()      => this.emit(MiniacEvent.Player_Prev)
        });

        this.server.use(this.signalR.createListener());
